...
export default function PageWrapper({
    ...
    return (
        <div className="container mx-auto px-4 py-16 flex items-center flex-col gap-16">
            ...
            {qa && (
                <section className="w-full max-w-2xl border border-border rounded-lg shadow-sm p-4">
            ...
            {stats?.length ? (
                <ul className="border border-border rounded-lg shadow-sm flex gap-0">
                    {stats.map((stat, idx) => (
                        <li
                            key={idx}
                            className="w-64 h-24 px-4 py-3 flex flex-col items-center gap-1"
                        >
